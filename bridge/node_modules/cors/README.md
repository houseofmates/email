...
var corsOptions = {
  origin: 'http://{{alias_domain}}',
  optionsSuccessStatus: 200 // some legacy browsers (IE11, various SmartTVs) choke on 204
}

// Adds headers: Access-Control-Allow-Origin: http://{{alias_domain}}, Vary: Origin
...
* `origin`: Configures the **Access-Control-Allow-Origin** CORS header. Possible values:
  ...
  - `String` - set `origin` to a specific origin. For example, if you set it to
    - `"http://{{alias_domain}}"` only requests from "http://{{alias_domain}}" will be allowed.
    - `"*"` for all domains to be allowed. 
  - `RegExp` - set `origin` to a regular expression pattern which will be used to test the request origin. If it's a match, the request origin will be reflected. For example the pattern `/example\.com$/` will reflect any request that is coming from an origin ending with "{{alias_domain}}".
...
### "Setting `origin: 'http://{{alias_domain}}'` means only that domain can access my server"

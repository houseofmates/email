...
COPY . .
RUN RUSTFLAGS="$(cat /flags.txt)" cargo build --target "$(cat /target.txt)" --release -p email --no-default-features --features "sqlite postgres mysql rocks s3 redis azure nats enterprise"
...
RUN export DEBIAN_FRONTEND=noninteractive && \
    ...
    groupadd -r -g 2000 email && \
    useradd -r -u 2000 -g 2000 -s /usr/sbin/nologin -M email && \
    mkdir -p /etc/email /var/lib/email && \
    chown email:email /etc/email /var/lib/email
COPY --from=builder --chmod=0755 /output/email /usr/local/bin/email
RUN setcap 'cap_net_bind_service=+ep' /usr/local/bin/email
USER email
WORKDIR /var/lib/email
VOLUME ["/etc/email", "/var/lib/email"]
...
ENTRYPOINT ["/usr/local/bin/email"]
CMD ["--config", "/etc/email/config.json"]
